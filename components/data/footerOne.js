const footerOne = {
    logo: '/assets/img/logo-9.png',
    title_1: 'About Us',
    description: 'Subscribe to out newsletter today to receive updates on the latest news, releases and special offers respect your privacy.',
    title_2: 'Other Pages',
    widgetMenus: [
        {link:'/about',title:'About Us'},
        {link:'/contact',title:'Contact Us'},
        {link:'/rooms',title:'Room List'},
        {link:'/terms',title:'Terms & Conditions'},
        {link:'/privacy',title:'Privacy Policy'},
        {link:'/cancellation',title:'Cancellation Policy'},
        {link:'/refund',title:'Refund Policy'},
    ],
    title_3: 'Gallery',
    title_4: 'Office Info',
    office_des: '1234 Lake Tahoe Blvd, South Lake Tahoe , California USA',
    officeInfos: [
        {link:'https://www.google.com/maps',info:'PGP+28Q Hotel Hillview, Block No 7 ,SY No 17, Hill Road, Madikeri, Karnataka 571201',icon: <i className="fal fa-map-marked-alt"></i>},
        {link:'mailto:coorghillview@gmail.com',info:'coorghillview@gmail.com',icon: <i className="fal fal fa-envelope-open-text"></i>},
        {link:'tel:+918310815401',info:'+91-8310815401',icon: <i className='fal fa-phone-alt'></i>},
        {link:'#',info:'+91-9448422334',icon: <i className="fab fa-whatsapp"></i>}
    ],
  }
  export default footerOne;